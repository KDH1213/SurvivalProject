...
public class PlacementUIController : MonoBehaviour
{
    ...
    [SerializeField]
    private DestoryCheckUI destoryCheckUI; // 파괴 UI
    [SerializeField]
    private RestUI restUI; // 휴식 UI
    [SerializeField]
    private TurretUI turretUI; // 포탑 UI
    [SerializeField]
    private BasePointUI basePointUI; // 거점 UI
    ...
    public void SetObjectList(PlacementObjectList database)
    {
        ...
        if (objectcontents.Count <= 0)
        {
            ...
            {
                var structureData = DataTableManager.StructureTable.Get(data.ID);
                if(data.Rank != 1)
                {
                    continue;
                }
