...
public class SkillUIView : MonoBehaviour, ISaveLoadData
{
    ...
    {
        if(skillTypeList.Count == 0 || skillTypeList.Count < (int)NormalSkillType.End)
        {
            for (int i = skillTypeList.Count; i < (int)NormalSkillType.End; ++i)
            {
                skillTypeList.Add(i);
            }
        }
    ...
    {
        for (int i = (int)NormalSkillType.End - 1; i >= 0; --i)
        {
